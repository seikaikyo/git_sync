...
handle_single_platform() {
    ...
    case $platform_choice in
        ...
        1)
            ...
            echo -e "${YELLOW}推送到 $platform_name (${existing_remote})...${NC}"

            if [ ! -z "$current_branch" ]; then

                git push "$existing_remote" "$current_branch"

            else

                git push "$existing_remote"
            ...
            if [ $? -eq 0 ]; then
                ...
                echo -e "${GREEN}✓ 成功推送到 $platform_name${NC}"

            else

                echo -e "${RED}✗ 推送到 $platform_name 失敗${NC}"
